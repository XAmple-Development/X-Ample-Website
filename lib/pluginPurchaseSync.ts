...
export async function syncPurchasesFromPlugin(triggeredBy: "admin" | "cron"): Promise<PluginSyncResult> {
  ...
  try {
    ...
    await setSystemState("plugin_sync", {
    ...
    return { upserted, scanned: payments.length };
  } catch (e) {
