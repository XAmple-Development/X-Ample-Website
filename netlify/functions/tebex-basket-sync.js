...
exports.handler = async (event) => {
  ...
  const accountToken = process.env.TEBEX_ACCOUNT_TOKEN; // <-- this should be your v3c-... public token
  ...
  // Optionally resolve username_id from sessionToken (Supabase)
  if (!resolvedUsernameId && sessionToken) {
    ...
    if (data?.username_id) resolvedUsernameId = data.username_id;
  }

  let basketIdent = ident || null;

  try {
    // 1) Create basket if needed
    if (!basketIdent) {
    ...
    // If caller only wants basket ident (pre-login), allow that.
    if (authOnly && !resolvedUsernameId) {
      return json(200, { ident: basketIdent });
    }

    // 2) If no username_id, they need to login before adding packages
    if (!resolvedUsernameId) {
      return json(409, {
        error: "Login required",
        ident: basketIdent,
        details: "No username_id available yet. Complete Tebex login first, then retry.",
      });
    }

    // 3) If no items, just return basket
    ...
    for (const item of items) {
      const addRes = await fetch(`${tebexBase}/accounts/${accountToken}/baskets/${basketIdent}/packages`, {
        method: "POST",
        headers: { "Content-Type": "application/json", Authorization: authHeader },
        body: JSON.stringify({
