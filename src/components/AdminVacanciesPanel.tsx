...
const AdminVacanciesPanel: React.FC = () => {
  ...
  useEffect(() => {
    const fetchAll = async () => {
      ...
      if (error) {
        toast({ title: "Failed to load vacancies", description: error.message, variant: "destructive" });
      } else if (data) {
        setItems((data as unknown) as VacancyRow[]);
      }
    };
    fetchAll();
  }, [toast]);
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    if (error) {
      toast({ title: "Failed to create vacancy", description: error.message, variant: "destructive" });
    } else {
...
export default AdminVacanciesPanel;
